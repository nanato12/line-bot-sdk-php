...
class WebhookEndpointTest extends TestCase
{
    ...
    public function testUpdateWebhookEndpoint()
    {
        ...
        $bot = new LINEBot(new DummyHttpClient($this, $mock), ['channelSecret' => 'CHANNEL-SECRET']);
        $res = $bot->updateWebhookEndpoint('https://example.herokuapp.com/test');

        $this->assertEquals(200, $res->getHTTPStatus());
        $this->assertTrue($res->isSucceeded());
    }
    public function testValidateWebhookEndpoint()
    {
        $mock = function ($testRunner, $httpMethod, $url) {
            /** @var \PHPUnit\Framework\TestCase $testRunner */
        ...
        $bot = new LINEBot(new DummyHttpClient($this, $mock), ['channelSecret' => 'CHANNEL-SECRET']);
        $res = $bot->validateWebhookEndpoint('https://example.herokuapp.com/test');
